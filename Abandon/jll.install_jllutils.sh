...
#   ModifiedTime: 2017-05-10 14:28:04
...
for((i=0; i<__arrCnt; i++)) {
    if [ -e "$(pwd)/jllutils/${__arrList[i]}" ]; then
        cp -rvf $(pwd)/jllutils/${__arrList[i]}  ${HOME}/.jllsystem/bin/ 
    else
        echo "JLL: Error due to $(pwd)/jllutils/${__arrList[i]} is not present"
    fi
}
