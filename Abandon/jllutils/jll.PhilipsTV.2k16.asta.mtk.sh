...
cat >&1 <<EOF
      ...
      # product     :  product name (QM16XE_F or QM16XE_U or QM16XE_UB or philips_MT5593Uplus_EU)
      # variant     :  [r]etail
      # content     :
      #                [f]ull upgrade image
      #                [t]ools (Copy tvsetting like) UPGs
      #
      # Example1    :  ./upgmaker.sh QM16XE_F r r -- 2K16 FHT - All UPG's
      # Example2    :  ./upgmaker.sh QM16XE_F r f -- 2K16 FHT - Only Full Upgrade UPG
...
if [ x"${GvResult}" = x"${GvMenuUtilsContent[GvResultID++]}" ]; then
    ...
    while [ ${isLooping} -eq 1 ]; do 
        ...
        if [ x"$b" = x ]; then
            echo
            echo -e "JLL: set ${TagPTN}${a}.${CvAccOff}${CvBgYellow}${CvFgBlack}bbb${CvAccOff}.ccc.ddd ; default by 10, * (Exit by press [q])"
