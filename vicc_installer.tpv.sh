...
EOF
    ...
    if [ ! -e "${GvReleasePath}/vicc" ]; then
        ...
        if [ -e "${GvReleasePath}/vicc/install.sh" ]; then
            ...
            echo "HOME=${HOME}"
            echo
            if [ -e "${HOME}/.jllsystem/bin" ]; then
                echo "JLL-IDE: Install TPV/PhilipsTV Tools..."
                [ x"$(ls ${CvScriptPath}/jllutils)" != x ] && \
                    cp -rvf ${CvScriptPath}/jllutils/*   ${HOME}/.jllsystem/bin/
                chmod +x -R ${HOME}/.jllsystem/bin
            fi 
